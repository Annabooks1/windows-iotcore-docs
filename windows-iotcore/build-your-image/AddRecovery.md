...
- Recovery customizations files (optional)
    - `RecoveryGUI.exe` : Optional simple UI to hide the recovery shell prompt on the device. Note: RecoveryGUI.exe can be a C++    application built for the specific CPU type or a .NET Framework Windows Form Application. Newwinpe.cmd would have to be modified to add .NET Framework 4 components to the WinPE image. 
